...
object RocksDbConfiguration {
  ...
  def apply(location: String): RocksDbConfiguration =
    apply(location, new Options().setCreateIfMissing(true))

  def apply(location: String, options: Options): RocksDbConfiguration =
    new RocksDbConfiguration(location,
                             BATCH_SIZE,
                             false,
                             MEMTABLE_SIZE,
                             DESIRED_NUM_FILES_AT_BASE_LEVEL,
                             MAX_BYTES_FOR_BASE_LEVEL,
