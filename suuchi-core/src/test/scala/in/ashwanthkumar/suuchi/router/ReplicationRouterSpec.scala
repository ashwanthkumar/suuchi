...
class NoReplicator(nrOfReplicas: Int, self: MemberAddress)
    extends ReplicationRouter(nrOfReplicas, self) {
  override def replicate[ReqT, RespT](eligibleNodes: scala.List[MemberAddress],
                                      serverCall: ServerCall[ReqT, RespT],
                                      headers: Metadata,
                                      incomingRequest: ReqT,
                                      delegate: ServerCall.Listener[ReqT]): Unit = {}
  override def doReplication[ReqT, RespT](
      eligibleNodes: scala.List[_root_.in.ashwanthkumar.suuchi.cluster.MemberAddress],
      serverCall: _root_.io.grpc.ServerCall[ReqT, RespT],
      headers: _root_.io.grpc.Metadata,
      incomingRequest: ReqT,
      delegate: _root_.io.grpc.ServerCall.Listener[ReqT]): Unit = ???
}

class MockReplicator(nrOfReplicas: Int, self: MemberAddress, mock: ReplicationRouter)
    extends ReplicationRouter(nrOfReplicas, self) {

  /**
   * @inheritdoc
   */
  override def replicate[ReqT, RespT](
      eligibleNodes: scala.List[_root_.in.ashwanthkumar.suuchi.cluster.MemberAddress],
      serverCall: _root_.io.grpc.ServerCall[ReqT, RespT],
      headers: _root_.io.grpc.Metadata,
      incomingRequest: ReqT,
...
class ReplicationRouterSpec extends FlatSpec {
  "ReplicationRouter" should "delegate the message to the local node if it's a REPLICATION_REQUEST" in {
    ...
    setupAndVerify {
      (serverCall: ServerCall[Int, Int],
       delegate: ServerCall.Listener[Int],
       next: ServerCallHandler[Int, Int]) =>
    ...
    setupAndVerify {
      (serverCall: ServerCall[Int, Int],
       ...
       next: ServerCallHandler[Int, Int]) =>
        ...
        verify(delegate, times(1)).onReady()
        verify(delegate, times(0)).onMessage(1)
        verify(mockReplicator, times(1)).replicate[Int, Int](
          any(classOf[List[MemberAddress]]),
          any(classOf[ServerCall[Int, Int]]),
          any(classOf[Metadata]),
          anyInt(),
          any(classOf[ServerCall.Listener[Int]]))
    }
  }

  def setupAndVerify(verify: (ServerCall[Int, Int],
                              ServerCall.Listener[Int],
                              ServerCallHandler[Int, Int]) => Unit): Unit = {
