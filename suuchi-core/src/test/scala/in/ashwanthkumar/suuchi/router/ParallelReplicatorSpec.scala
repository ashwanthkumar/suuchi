...
class MockParallelReplicator(nrReplicas: Int, self: MemberAddress, mock: ParallelReplicator)(
    implicit executor: Executor)
    extends ParallelReplicator(nrReplicas, self) {
  override def forwardAsync[RespT, ReqT](methodDescriptor: MethodDescriptor[ReqT, RespT],
                                         headers: Metadata,
...
class ParallelReplicatorSpec extends FlatSpec {
  "ParallelReplicator" should "forward requests to target nodes in parallel" in {
    ...
    when(
      mockReplicator.forwardAsync(any(classOf[MethodDescriptor[Int, Int]]),
    ...
    verify(mockReplicator, times(1)).forwardAsync(any(classOf[MethodDescriptor[Int, Int]]),
                                                  any(classOf[Metadata]),
                                                  anyInt(),
                                                  Matchers.eq(destination1))
    verify(mockReplicator, times(1)).forwardAsync(any(classOf[MethodDescriptor[Int, Int]]),
