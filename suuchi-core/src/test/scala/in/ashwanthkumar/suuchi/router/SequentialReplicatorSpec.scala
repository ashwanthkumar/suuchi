...
class TestSequentialReplicator(nrReplicas: Int, self: MemberAddress)
    extends SequentialReplicator(nrReplicas, self) {
  override def forward[RespT, ReqT](methodDescriptor: MethodDescriptor[ReqT, RespT],
                                    headers: Metadata,
                                    incomingRequest: ReqT,
                                    destination: MemberAddress): Any = {}
}

class MockSequentialReplicator(nrReplicas: Int, self: MemberAddress, mock: SequentialReplicator)
    extends SequentialReplicator(nrReplicas, self) {
  override def forward[RespT, ReqT](methodDescriptor: MethodDescriptor[ReqT, RespT],
                                    headers: Metadata,
                                    incomingRequest: ReqT,
...
class SequentialReplicatorSpec extends FlatSpec {
  "SequentialReplicator" should "fail if number of nodes is < expected replicas" in {
    ...
    replicator
      .replicate[Int, Int](List(MemberAddress("host1", 1)), serverCall, headers, 1, delegate)
    ...
    actualStatus.getDescription should be(
  ...
  it should "sequentially send forwards to the replicas" in {
    ...
    replicator
      .replicate[Int, Int](List(destination1, destination2), serverCall, headers, 1, delegate)

    verify(mockReplicator, times(1)).forward(any(classOf[MethodDescriptor[Int, Int]]),
                                             any(classOf[Metadata]),
                                             anyInt(),
                                             Matchers.eq(destination1))
    verify(mockReplicator, times(1)).forward(any(classOf[MethodDescriptor[Int, Int]]),
                                             any(classOf[Metadata]),
                                             anyInt(),
                                             Matchers.eq(destination2))
  }
  ...
  it should "call delegate.OnMesssage if one of the nodes to replica is self" in {
    ...
    verify(mockReplicator, times(0)).forward(any(classOf[MethodDescriptor[Int, Int]]),
                                             any(classOf[Metadata]),
                                             anyInt(),
                                             Matchers.eq(destination1))
    verify(mockReplicator, times(1)).forward(any(classOf[MethodDescriptor[Int, Int]]),
