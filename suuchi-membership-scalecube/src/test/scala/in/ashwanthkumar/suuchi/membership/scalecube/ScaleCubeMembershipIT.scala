...
import in.ashwanthkumar.suuchi.membership.{InMemorySeedProvider, MemberAddress, MemberListener, Membership}
...
class ScaleCubeMembershipIT extends FlatSpec with BeforeAndAfter {
  ...
  "ScaleCubeCluster" should "launch 5 nodes and say they have 5 nodes" in {
    ...
    val seedNode = new ScaleCubeMembership(BASE_PORT + 1, Some(gossipConfig),
      List(
    ...
    (2 to 5).foreach { i =>
      ...
      val member = new ScaleCubeMembership(BASE_PORT + i, Some(gossipConfig), Nil)
