...
case class ExpectedMemberCount(joinLatch: Option[CountDownLatch] = None,
                               removeLatch: Option[CountDownLatch] = None)
    extends MemberListener {
...
class ScaleCubeClusterIT extends FlatSpec with BeforeAndAfter {
  ...
  "ScaleCubeCluster" should "launch 5 nodes and say they have 5 nodes" in {
    ...
    (2 to 5).foreach { i =>
      ...
      val member = ClusterProvider.apply(MemberAddress("localhost", BASE_PORT + i),
                                         configWithPort(BASE_PORT + i),
                                         Nil)
